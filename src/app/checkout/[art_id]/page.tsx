  ...
  /* useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      window.payhere.onCompleted = async function onCompleted(orderId) {
        ...
        // The Supabase UPDATE to 'paid' goes here!
        const { error } = await supabase.from('orders').update({ status: 'paid' }).eq('order_id', orderId);
        if (!error) {
           alert("Payment Success! Database updated to PAID. ");
           router.push('/artworks');
        }
      };

      // @ts-ignore
      window.payhere.onDismissed = function onDismissed() { console.log("Dismissed"); };
      // @ts-ignore
      window.payhere.onError = function onError(error) { console.log("Error:" + error); };
    }
  }, [router]);
   */
